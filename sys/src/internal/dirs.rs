...
pub const SHERPA_ROOT: &str = "sherpa-rs";
...
pub fn cache_dir() -> Option<std::path::PathBuf> {
    self::windows::known_folder_local_app_data().map(|h| h.join(SHERPA_ROOT))
}
...
pub fn cache_dir() -> Option<std::path::PathBuf> {
    std::env::var_os("XDG_CACHE_HOME")
        ...
        .or_else(|| self::unix::home_dir().map(|h| h.join(".cache").join(SHERPA_ROOT)))
}

#[cfg(target_os = "macos")]
#[must_use]
pub fn cache_dir() -> Option<std::path::PathBuf> {
    self::unix::home_dir().map(|h| h.join("Library/Caches").join(SHERPA_ROOT))
}
