...
#[derive(Debug)]
pub struct OmnilingualRecognizer {
    recognizer: *const sherpa_rs_sys::SherpaOnnxOfflineRecognizer,
}

pub type OmnilingualRecognizerResult = super::OfflineRecognizerResult;
...
impl OmnilingualRecognizer {
    pub fn new(config: OmnilingualConfig) -> Result<Self> {
        ...
        let model_config = unsafe {
            sherpa_rs_sys::SherpaOnnxOfflineModelConfig {
                ...
                omnilingual: sherpa_rs_sys::SherpaOnnxOfflineOmnilingualAsrCtcModelConfig {
        ...
        if recognizer.is_null() {
            bail!("Failed to create Omnilingual recognizer");
        }

        Ok(Self { recognizer })
    }

    pub fn transcribe(&mut self, sample_rate: u32, samples: &[f32]) -> OmnilingualRecognizerResult {
        unsafe {
            ...
            let result = OmnilingualRecognizerResult::new(&raw_result);
            // Free
...
unsafe impl Send for OmnilingualRecognizer {}
unsafe impl Sync for OmnilingualRecognizer {}

impl Drop for OmnilingualRecognizer {
    fn drop(&mut self) {
        unsafe {
